import { MovieCategory } from './movie-category';

export const HomeCategories: MovieCategory[] = [
  { category: 'Fastflix 인기 콘텐츠', movies: [] },
  { category: '내가 찜한 콘텐츠', movies: [] },
  { category: '특별 소개 콘텐츠', movies: [] },
  { category: '최신 등록 콘텐츠', movies: [] },
  { category: '시청 중인 콘텐츠', movies: [] },
  { category: 'Fastflix 오리지널', movies: [] },
  { category: 'ost가 좋은 영화', movies: [] },
  { category: '여름과 관련된 영화', movies: [] },
  { category: '미치도록 웃긴영화', movies: [] },
  { category: '영어 공부하기 좋은 영화', movies: [] },
  { category: '디즈니 영화', movies: [] },
  //우리만의 카테고리 추가될 것
];
